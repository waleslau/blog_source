...
先是找到了 libfaketime 这个工具，但这个库只对当时运行的二进制有关，影响不到正在运行的文件系统，或许有某种办法可以在挂载文件系统时或更早的阶段让 libfaketime 介入，但我感觉有点风险，为了数据安全就先放下了这个想法。
...
[^2]: 如果只是在挂载目录内操作，其实还是等同于直接在外面的文件系统操作，但外面的文件系统是已经运行在真实的时间下的，faketime 无法对它施加影响。
